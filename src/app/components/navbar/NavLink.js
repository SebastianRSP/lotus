        ...
        >
            {icon ? (
                <Image
                    ...
                    width={18}
                    height={15}
                    className={`fill-current w-6 h-6 md:w-4 md:h-4 ${isToggled ? 'fill-green' : ''}`}
                />
            ) : (
                <span className={`${font} ${isToggled ? 'text-green' : ''} text-black uppercase lg:text-sm md:text-xs font-medium md:leading-66 leading-none tracking-space60 inner-text`}>
                    {navLinkName}
                </span>
            )}
        </Link>
